...
<script>
  $(function() {
    ...
    $( "#dates" ).val( "$" + $( "#slider-range1" ).slider( "values", 0 ) +
...
<ul class="nav nav-pills">
		...
		<p>
  <label for="dates"><h3>Date range:</h3></label>
  <input type="text" id="dates" size="150" style="border: 0; color: #388E8E; font-weight: bold;" />
</p>
		...
		for (JobSeeker s : th) {
			if (s.postOwner.equals(u)) {
				...
				url="http://api.careerbuilder.com/v1/jobsearch?DeveloperKey=WDHT1YG61B6HPD3NGC2Y"+"&perPage=25&Keywords="+s.jobtype+"%2C"+interests;
					   ...
					   //for ( var i = 0; i < document.geo.count.value; i++ ) {
							$.each(urlArray, function (index, data) {	
								...
								$('#map_canvas').gmap('addMarker', { 'position': pos } ).mouseover(function() {
						            ...
						            $info.find("img").attr("alt", "hello").attr("src", data.replace('=s300', '=s100'));
